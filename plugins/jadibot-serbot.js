...
// ========== NUEVO SISTEMA DE REINICIO (desde el primer código) ==========
const retryMap = new Map(); 
const maxAttempts = 5;

async function checkSubBots() {
    const subBotDir = path.resolve(`./${jadi}`); // Uso la variable jadi que ya existe en tu contexto
    ...
    for (const folder of subBotFolders) {
        const pathkanekiAIJadiBot = path.join(subBotDir, folder);
        const credsPath = path.join(pathkanekiAIJadiBot, "creds.json");
